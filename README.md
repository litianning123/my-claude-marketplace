...
| [`dev-unslop`](dev-unslop/) | Removes AI-generated slop from technical writing — READMEs, PR replies, code comments, docs. Detects throat-clearing, filler adjectives, structural bloat, AI watermark phrases (30+ English + Chinese), and PR glazing. Applies a senior-engineer aesthetic: if a sentence does not carry load, it is cut. |
| [`loop-creator`](loop-creator/) | Wizard-style interview that generates ready-to-run `/loop` configurations. Discovers loop candidates from session history (repeated prompts, temporal cues), then outputs a command, project folder, or reusable skill. Encodes loop engineering best practices: readiness gate, permission ladder, maker-checker verification, and manual-before-scheduled discipline. |
